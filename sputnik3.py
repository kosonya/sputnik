...
class Sputnik:
	...
	def render(self, surface, color=(0,0,255)):
		...
		if len(self.future) >= 2: pygame.draw.lines(surface, (127, 127, 0), False, [   (x_y_xv_xy[0]*self.zoom + self.center_shift[0], x_y_xv_xy[1]*self.zoom + self.center_shift[1])   for x_y_xv_xy in self.future])
		if len(self.past) >= 2: pygame.draw.aalines(surface, color, False, [ (x_y[0]*self.zoom+self.center_shift[0], x_y[1]*self.zoom+self.center_shift[1]) for x_y in self.past])
...
def sma(r, v):
		global mu
		return 1/max( (2.0/r) - ( (v*v) / mu), 0.001)
...
class Label(object):
	...
	def set_value(self, value):
		self.val = self.font.render(self.form % value, True, self.color)
...
#	sputnik1 = Sputnik((g_point[0]+planet_r+alt,g_point[1]), g_point, (-vy, vx), planet_r)
	...
	faillabel = failfont.render("You will fall!", True, (255, 0, 0))
	failrect = faillabel.get_rect()
	failrect.midbottom = screen.get_rect().midbottom

	pygame.draw.circle(trace, (0,0,255), g_point, planet_r)
	for _ in range(70):
		r = random.randrange(2, int(planet_r/4))
		dr = random.randrange(1, int(planet_r - r))
